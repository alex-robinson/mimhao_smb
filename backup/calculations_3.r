...
for (i in 1:length(index)){
    smb_racmo_bm = sum(smb_racmo.year*region_mask[[i]]$ice*area)/1e12*365 #NOT USEFUL
    df[i,"SMB-1981-2010"] = smb_racmo_bm #2nd COLUMN OF THE df TABLE (real data from reanalysis) #NOT USEFUL
    #smb = sapply(SMB.year, function(x) sum(x*region_mask[[i]]$ice*area)/1e12 * 365) #As it comes from SMB.month, smb has 3 matrix inside too
    smb = sapply(SMB.year, function(x) sum(x*region_mask[[i]]$ice))
    #df[i,"SMB-A1B-2000-2010"] = smb[1] #3rd COLUMN OF THE df TABLE (it is the reference for the A1B scenario simulation)
    df[i,"SMB-A1B-2000-2010"] = smb[1]*df[i,"Area_bedmap"]*365*1e6/1e12 #3rd COLUMN OF THE df TABLE (it is the reference for the A1B scenario simulation)
...
for (i in 1:length(index)){
    ...
    df_2[i,"bmelt(m/year)"] = mean((bmelt*region_mask[[i]]$ice)[ii])
    df_2[i,"bss(m/year)"] = mean((bss*region_mask[[i]]$ice)[ii])
}

#Basal melt in Gt/year
for (i in 1:length(index)){
  # ii = which(region_mask[[i]]$ice == 1)
  # df_2[i,"bmelt(Gt/year)"] = mean((bmelt*region_mask[[i]]$ice*area*916.8/1e12)[ii])
  # df_2[i,"bss(Gt/year)"] = mean((bss*region_mask[[i]]$ice*area*916.8/1e12)[ii])
...
gl_flux_u[,,i] <- (regions_uv(region_mask[[i]]$land))$u * 40e3 *VEL_R11$u * h * 916.8 /1e12 #Gt/year
gl_flux_v[,,i] <- (regions_uv(region_mask[[i]]$land))$v * 40e3 *VEL_R11$v * h * 916.8 /1e12 #Gt/year
...
for (i in 1:length(index)){
#Ice density at 0ºC is (aproximately) ---------------------------------------------> 916,8 kg/m³
if_flux_u[,,i] <- (regions_uv(region_mask[[i]]$ice))$u * 40e3 *VEL_R11$u * h * 916.8 /1e12 #Gt/year
if_flux_v[,,i] <- (regions_uv(region_mask[[i]]$ice))$v * 40e3 *VEL_R11$v * h * 916.8 /1e12 #Gt/year
#I have to fix this part of the loop...
...
df_3[,"SMB Gt/year"] = df[,"SMB-A1B-2000-2010"] #3rd COLUMN: 3rd COLUMN of the df TABLE (it is the reference for the A1B scenario simulation)
df_3[,"Ice front Gt/year"] = df_if[,"icefront"] #4th COLUMN
df_3[,"dH/dt Gt/year"] = df_gl["groundline-bm"]+df[,"SMB-A1B-2000-2010"]-df_if[,"icefront"]-df_2[,"bmelt(Gt/year)"] #5th COLUMN #CHANGED
